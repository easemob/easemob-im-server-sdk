...
public class EMMessageContent {
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(msg, filename, secret, size, url, length, thumb, fileLength, thumbSecret, lat, lng, addr, action, customEvent, customExts);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EMMessageContent.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in EMMessageContent is not found in the empty JSON string", EMMessageContent.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EMMessageContent.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EMMessageContent` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("thumb_secret") != null && !jsonObj.get("thumb_secret").isJsonNull()) && !jsonObj.get("thumb_secret").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `thumb_secret` to be a primitive type in the JSON string but got `%s`", jsonObj.get("thumb_secret").toString()));
      }
      ...
      if ((jsonObj.get("lng") != null && !jsonObj.get("lng").isJsonNull()) && !jsonObj.get("lng").isJsonPrimitive()) {
