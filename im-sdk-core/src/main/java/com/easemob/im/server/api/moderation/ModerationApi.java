...
public class ModerationApi {
     ...
     * @param filename 下载到哪个文件，可以理解为带后缀的文件名，建议使用 .txt为后缀
     * @return path
     */
    public Mono<Path> download(String uuid, Path dir, String filename) {
        return this.recordFileDownload.toLocalFile(uuid, dir, filename);
    }
}
