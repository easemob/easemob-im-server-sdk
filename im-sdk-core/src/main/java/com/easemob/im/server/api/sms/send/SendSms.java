    ...
    public Mono<Integer> send(Set<String> phoneNumbers, String smsTemplateId, Map<String, String> templateVariableContent, String extendCode, String customAttributes) {
        return this.context.getHttpClient()
                .flatMap(httpClient -> httpClient.headersWhen(headers -> Mono.just(headers.add("Content-Type", "application/json")))
                        .post()
                        .uri("/sms/send")
                        .send(Mono.create(sink -> sink.success(this.context.getCodec().encode(new SendSmsRequest(phoneNumbers, smsTemplateId, templateVariableContent, extendCode, customAttributes)))))
