...
public class CreateCmd {
    ...
    public void block(@Parameters(description = "user to block") String username,
        ...
        if (StringUtils.hasText(argGroup.msgToGroupId)) {
            this.service.block()
                    .blockUserSendMsgToGroup(username, argGroup.msgToGroupId, argGroup.duration)
                    .doOnSuccess(ignore -> System.out.println("done"))
                    .doOnError(err -> System.out.println("error: " + err.getMessage()))
                    .onErrorResume(EMException.class, ignore -> Mono.empty())
                    .block();
        ...
        if (StringUtils.hasText(argGroup.msgToRoomId)) {
            this.service.block()
                    .blockUserSendMsgToRoom(username, argGroup.msgToRoomId, argGroup.duration)
    ...
    public void group(@Parameters(arity = "1", description = "the member's username list")
            List<String> members,
            @Option(names = "--owner", required = true, description = "the owner's username")
                    String owner,
            @Option(names = "--name", required = true, description = "the group's name")
                    String name,
            @Option(names = "--description", required = true, description = "the group's description")
                    String description,
            @Option(names = "--private", description = "create a private group") boolean isPrivate,
            @Option(names = "--max-members", defaultValue = "200", description = "max number of members")
                    int maxMembers,
            @Option(names = "--can-member-invite", description = "can member invite others to join")
                    boolean canMemberInvite) {
        if (isPrivate) {
            this.service.group().createPrivateGroup(owner, name, description, members, maxMembers,
                    canMemberInvite)
            ...
            this.service.group().createPublicGroup(owner, name, description, members, maxMembers,
                    !canMemberInvite)
                    .doOnSuccess(groupId -> System.out.println("group: " + groupId))
                    .doOnError(err -> System.out.println("error: " + err.getMessage()))
                    .onErrorResume(EMException.class, ignore -> Mono.empty())
    ...
    public void room(@Parameters(arity = "1", description = "the member's username list")
            List<String> members,
            @Option(names = "--name", required = true, description = "the room name") String name,
            @Option(names = "--owner", required = true, description = "the owner's username")
                    String owner,
            @Option(names = "--description", description = "the room's description", defaultValue = "")
                    String description,
            @Option(names = "--max-members", defaultValue = "200", description = "max number of members")
                    int maxMembers) {
    ...
    public void message(
            @Option(names = "--from", description = "message sender username", required = true)
                    String from,
            @ArgGroup(multiplicity = "1", heading = "To whom.\n") MessageToGroup to,
            @ArgGroup(multiplicity = "1", heading = "Message type\n") MessageTypeArgGroup msg,
            @Option(names = "--secret", description = "secret returned by attachment upload service")
                    String secret,
            @Option(names = "--filename", description = "filename", defaultValue = "未命名")
                    String filename,
            @Option(names = "--bytes", description = "file size, unit is B(byte)", defaultValue = "0")
                    Integer bytes,
            @Option(names = "--duration", description = "video or audio duration, unit is s(second)", defaultValue = "0")
                    Integer duration,
            @Option(names = "--ext", description = "message extension")
                    Map<String, Object> extensions,
            @Option(names = "--param", description = "message extension")
        ...
        if (msg.text != null) {
            ...
            message = new EMImageMessage().displayName(filename).uri(URI.create(msg.img))
                    .secret(secret).bytes(bytes);
        } else if (msg.audio != null) {
            message = new EMVoiceMessage().displayName(filename).uri(URI.create(msg.audio))
                    .secret(secret).duration(duration).bytes(bytes);
        } else if (msg.video != null) {
            message = new EMVideoMessage().displayName(filename).uri(URI.create(msg.video))
                    .secret(secret).duration(duration).bytes(bytes);
        } else if (msg.loc != null) {
            message = new EMLocationMessage().longitude(Double.parseDouble(msg.loc[0]))
                    .latitude(Double.parseDouble(msg.loc[1])).address(msg.loc[2]);
        } else if (msg.file != null) {
            message = new EMFileMessage().displayName(filename).uri(URI.create(msg.file))
                    .secret(secret).bytes(bytes);
        } else if (msg.cmd != null) {
            message = new EMCommandMessage().action(msg.cmd).params(EMKeyValue.of(params));
        } else {
            message = new EMCustomMessage().customEvent(msg.custom)
                    .customExtensions(EMKeyValue.of(params));
        }
        String toType = to.toUsernames != null ?
                "users" :
                to.toGroupIds != null ? "chatgroups" : "chatrooms";
        Set<String> toIds = to.toUsernames != null ?
                to.toUsernames :
                to.toGroupIds != null ? to.toGroupIds : to.toRoomIds;
    ...
    private static class AdminArgGroup {
        @Option(names = "--group", description =
                "promote the specified user to group admin, group admin is some user who can block user from joining or "
                        +
                        "sending messages in this group")
        String groupId;

        @Option(names = "--room", description =
                "promote the specified user to room admin, room admin is some user who can block user from joining or "
                        +
